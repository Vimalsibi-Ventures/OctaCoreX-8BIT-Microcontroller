...
entity risc_core_top is
    Port (
        ...
        monitor_result : out STD_LOGIC_VECTOR(DATA_WIDTH-1 downto 0)
    );
end risc_core_top;
...
architecture Structural of risc_core_top is
    ...
    signal wire_hazard_stall, wire_hazard_flush, wire_pc_write, wire_if_id_write, wire_branch_taken : STD_LOGIC;
    ...
    signal wire_alu_zero : STD_LOGIC;
...
begin
    ...
    U_HAZARD: hazard_unit PORT MAP (
        ...
        branch_taken => wire_branch_taken, pc_write => wire_pc_write, 
    ...
    U_ALU: alu_8bit PORT MAP (in_a => id_ex_reg_data1, in_b => wire_alu_b_in, alu_ctrl => id_ex_alu_ctrl, alu_res => wire_alu_res, zero_flag => wire_alu_zero);
    U_DATA_RAM: data_ram PORT MAP (clk => clk, we => ex_mem_mem_write, re => ex_mem_mem_read, addr => ex_mem_alu_res, data_in => ex_mem_write_data, data_out => wire_ram_out);

    wire_wb_data <= mem_wb_ram_data when mem_wb_mem_to_reg = '1' else mem_wb_alu_res;
    
    -- Fixed Branch Logic: Evaluated perfectly in EX stage
    wire_branch_taken <= '1' when (id_ex_branch = '1' and (id_ex_alu_ctrl = OP_JMP or (id_ex_alu_ctrl = OP_JZ and id_ex_reg_data1 = x"00"))) else '0';

    pc_out <= std_logic_vector(pc_reg); monitor_result <= wire_wb_data;
